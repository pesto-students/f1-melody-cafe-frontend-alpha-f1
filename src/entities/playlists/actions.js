...
const searchInitializePlaylist = (
  id,
  genre,
  thumbnail,
  playlistTitle,
...
const grabPlaylistData = (id, genre, nextPageToken, getState, dispatch) => {
  ...
  if (state.playlists.cachedPlaylists[genre]) {
    if (state.playlists.cachedPlaylists[genre][id]) {
      if (
        state.playlists.cachedPlaylists[genre][id].songs.length ===
        state.playlists.cachedPlaylists[genre][id].playlistInfo.songCount
      ) {
        return;
      }
    }
      ...
      .then((response) => {
        ...
        dispatch(
          initializePlaylist(
            ...
            genre,
            state,
            response.data.items[0].contentDetails.itemCount,
            thumbnail
          )
        );
      })
...
export { onPageSelect, onPlaylistSelect, onPlaylistInit, onSearchPlaylistInit };
